...
interface FilterPopoverProps {
  isOpen: boolean;
  onClose: () => void;
  id: string;  // Add an id prop to differentiate each filter instance
}

const AnalyticsFilter: React.FC<FilterPopoverProps> = ({ isOpen, onClose, id }) => {
  ...
  return (
    <div className="relative">
      ...
      {/* Popover */}
      <div className="absolute right-4 sm:right-0 top-0 w-80 z-[100] transform transition-all duration-200 ease-out animate-in fade-in slide-in-from-top-2">
            ...
            <div className="space-y-2">
              ...
              {expandedSections.date && (
                <div className="pl-6 space-y-2 animate-in slide-in-from-top-1">
                  <div className="space-y-2">
                    {/* From Date */}
                    <label htmlFor={`${id}-from`} className="text-sm text-gray-600">From</label>
                    <input 
                      id={`${id}-from`} // Unique ID for 'from' date input
                      type="date" 
                    ...
                    {/* To Date */}
                    <label htmlFor={`${id}-to`} className="text-sm text-gray-600">To</label>
                    <input 
                      id={`${id}-to`} // Unique ID for 'to' date input
                      type="date" 
                      className="w-full px-3 py-2 rounded-lg border border-gray-200 focus:border-blue-300 focus:ring-1 focus:ring-blue-300 transition-all"
                    />
                  </div>
